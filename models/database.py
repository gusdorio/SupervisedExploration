...
# Make MongoDB imports optional (for environments without MongoDB)
try:
    ...
    logger.warning("MongoDB libraries not available - MongoDB functionality disabled")
...
class DatabaseConfig:
    ...
    def get_mysql_url(cls) -> str:
        ...
        # Check if we should use SQLite (for environments without MySQL/Docker)
        use_sqlite = os.getenv('USE_SQLITE', 'false').lower() == 'true'
        if use_sqlite:
            db_path = os.getenv('SQLITE_PATH', 'data/icb_database.db')
...
class MongoDBConnection:
    ...
    _client = None  # Optional[MongoClient] when available
    _db = None
    ...
    def test_connection(self) -> bool:
        ...
        if not MONGODB_AVAILABLE or self._client is None:
